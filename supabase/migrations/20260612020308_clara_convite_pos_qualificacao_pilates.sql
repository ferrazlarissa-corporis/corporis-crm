-- Após a aluna explicar o motivo do Pilates, a Clara deve fazer o convite
-- direto para a aula experimental avaliativa, sem chamar de "avaliação gratuita"
-- e sem alongar em explicação clínica.
...
[
  ...
  },
  {
    ...
    "dialogo": [
      {
        "autor": "lead",
        "texto": "fico na mesma posição no trabalho, minha postura me incomoda um pouco, mas não tenho muita dor. mais para me movimentar mesmo"
      },
      {
        "autor": "clara",
        "texto": "Entendi, Ana. Então tenho um convite para te fazer: nós oferecemos uma aula experimental avaliativa."
      },
      {
        "autor": "clara",
        "texto": "Nesse encontro, entendemos o seu histórico, avaliamos os seus movimentos e você pode vivenciar no seu corpo como o pilates pode te ajudar nesse momento."
...
where not exists (
  ...
  where item ->> 'id' = 'nao-chamar-de-avaliacao-gratuita'
);
...
where not exists (
  select 1
  from jsonb_array_elements(c.boas_praticas) item
  where item ->> 'id' = 'convite-pos-qualificacao-pilates'
);
...
with updated as (
  select
    ...
    jsonb_agg(
      case
        when item.value ->> 'id' = 'convite-aula-experimental-avaliativa'
          then jsonb_set(
            item.value,
            '{detail}',
            to_jsonb('Use esse nome, explique que nesse encontro entendemos o histórico, avaliamos os movimentos e a aluna vivencia como o pilates pode ajudar. Depois pergunte se ela consegue durante o dia ou se fica melhor após às 18h.'::text),
            true
          )
        else item.value
      end
...
where not exists (
  ...
  where item ->> 'id' = 'larissa-convite-pos-postura-movimento'
);
